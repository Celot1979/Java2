...
class marco extends JFrame {
	...
	public marco() {
		...
		//new Thread(new Respuesta_Opciones_Servidor_Cliente("192.168.0.7", 9995)).start();
		...
		class Socket_Inicio{
			public Socket_Inicio(String Ip, int puerto) {
				try {
					...
					try (Socket socket_envio_Info = new Socket(Ip,puerto)) {
  ...
  //------------------------------------------- EXISTEN 4 OPCIONES Y SE LE ENVÍA LA INFO --------------------------------------------------------------
    ...
    public class hilo_Escucha_Info_Servidor_Cliente implements Runnable{
    	...
    	class Escucha_Servidor{
			...
			public Escucha_Servidor(int puerto) throws IOException, ClassNotFoundException {
    			try (ServerSocket Servidor_Escucha = new ServerSocket(puerto)) {
					...
					while(true) {
						...
						if(datos_paquete.getOpcion().equals("1")) {
							@SuppressWarnings("unused")
							Socket respuesta = new Socket ("192.168.0.7",9991);
							@SuppressWarnings("unused")
							String Logus,mensaje,Opcio_elegida;
							@SuppressWarnings("unused")
							Informacion_Cliente datos_paquete1;
    ...
    //----------------------------------------------- HILO CONSTRUIDO PARA ENVIAR-----------------------------------------------------------------------
    //--------------------------------------------- INFORMACIÓN  (SERVIDOR - CLIENTE ) ------------------------------------------------------------------
    //----------------------------- EXISTEN 4 OPCIONES A LAS QUE HAY QUE CREAR UNA CONTESTACIÓN DEL CLIENTE ---------------------------------------------
    
    /*public class Respuesta_Opciones_Servidor_Cliente implements Runnable{
       
		public Respuesta_Opciones_Servidor_Cliente(String ip, int port) {
			Ip = ip;
			Port = port;
		}
		@Override
		public void run() {
			// TODO Auto-generated method stub
			try {
				Respuesta Contestacion_respuesta = new Respuesta(Ip ,Port);
			} catch (UnknownHostException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (IOException e) {
    	...
    	class Respuesta {
			...
			public Respuesta(String ip, int port) throws UnknownHostException, IOException {
				Socket Socket_Contestacion_Opciones_Servidor_Cliente = new Socket(Ip ,Port);
				Informacion_Cliente Contestacion_Via_Paquete = new Informacion_Cliente();
				String mensaje_Uno = "\n" + Contestacion_Via_Paquete.getUsuario() + "\n" +  "Devil Sheep: " + "\n" + " En la mayor brevedad posible nos pondremos en contacto con usted vía email en realción a su pedido: " + "\n" +  Contestacion_Via_Paquete.getN_pedido() ;
				String mensaje_Dos = "\n" + Contestacion_Via_Paquete.getUsuario() + "\n" +  "Devil Sheep: " + "\n" + " Podrá encontrar toda la información sobre devoluciones en nuestra web: http//:DevilSheep.com/Devoluciones " + "\n" +  "Devoluciones recogidas sin conste, y un plazo máximo para la devolución de 180 días" ;
				String mensaje_Tres = "\n" + Contestacion_Via_Paquete.getUsuario() + "\n" +  "Devil Sheep: " + "\n" + " En nuestra sección http//:DevilSheep.com/Contacto podrá encontrar todas nuestras tiendas ";
				
				if(Contestacion_Via_Paquete.getOpcion().equals("1")) {
					ObjectOutputStream flujosSalidaPaquete = new ObjectOutputStream(Socket_Contestacion_Opciones_Servidor_Cliente.getOutputStream());
					flujosSalidaPaquete.writeObject(mensaje_Uno);
					System.out.print("Ha llegado hasta el hilo de contestación del Servidor");
					Socket_Contestacion_Opciones_Servidor_Cliente.close();
				}else if(Contestacion_Via_Paquete.getOpcion().equals("2")) {
					ObjectOutputStream flujosSalidaPaquete = new ObjectOutputStream(Socket_Contestacion_Opciones_Servidor_Cliente.getOutputStream());
					flujosSalidaPaquete.writeObject(mensaje_Dos);
					Socket_Contestacion_Opciones_Servidor_Cliente.close();
				}else if(Contestacion_Via_Paquete.getOpcion().equals("3")) {
					ObjectOutputStream flujosSalidaPaquete = new ObjectOutputStream(Socket_Contestacion_Opciones_Servidor_Cliente.getOutputStream());
					flujosSalidaPaquete.writeObject(mensaje_Tres);
					Socket_Contestacion_Opciones_Servidor_Cliente.close();
				}else if(Contestacion_Via_Paquete.getOpcion().equals("4")) {
					String mensaje_Cuatro = JOptionPane.showInputDialog("Que quieres decir al cliente:");
				}
  ...
  //---------------------------------------------------------------------------------------------------------------------------------------------------
	...
	@SuppressWarnings("unused")
	private Informacion_Cliente datos_paquete;
	Socket Socket_Envio_Info_Cliente;
