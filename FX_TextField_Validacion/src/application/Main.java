...
public class Main extends Application {
	...
	private void btAccionClic() {
		...
		if(mensaje.length()==0) {
			if(comprobarEntero(txtPuntos,"Debes introduccir un valor númerico entero")) {
			mensaje = "El jugador " + txtJugador.getText() + " que juega en " + txtEquipo.getText() +
					" anota " + txtPuntos.getText() + " puntos por partido ";
			Ventana.mostrar(mensaje, "Datos del jugador");
			}
			else{txtPuntos.requestFocus();}
		}else {
			Ventana.mostrar(mensaje, "Faltan datos !!!");
		}
	...
	private boolean comprobarEntero(TextField f, String mensaje) {
		try{
			...
			Ventana.mostrar(mensaje, "Error en los datos!!!");
			return false;
		}
